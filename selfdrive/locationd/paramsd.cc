...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    read_db_value("CarParams", &value, &value_sz, false);
    if (value_sz > 0) break;
    usleep(100*1000);
  }
  ...
  const int result = read_db_value("LiveParameters", &value, &value_sz, false);
      ...
      if (which == cereal::Event::CAMERA_ODOMETRY){
        ...
        if (save_counter % 6000 == 0) {
          ...
          std::async(std::launch::async,
                     [out]{
                       write_db_value("LiveParameters", out.c_str(), out.length(), false);
                     });
        }
      }
      delete msg;
