...
void encoder_thread(bool is_streaming, bool raw_clips, bool front) {
  ...
  if (front) {
    char *value;
    const int result = read_db_value("RecordFront", &value, NULL, false);
    if (result != 0) return;
